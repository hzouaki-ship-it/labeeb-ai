...
# 2. حقن التنسيقات وإصلاح اتجاه الكتابة (RTL) عبر كتل برمجية معزولة وقصيرة تجنباً لأخطاء السحابة
...
st.markdown('<style>[data-testid="stMain"] .block-container { padding-top: 2rem !important; padding-bottom: 3rem !important; max-width: 850px !important; }</style>', unsafe_allow_html=True)
st.markdown('<style>[data-testid="stVerticalBlock"] { gap: 0rem !important; } h1, h2, h3, h4, h5, h6, p, span, label, table, th, td { font-family: "Cairo", sans-serif !important; text-align: right !important; direction: rtl !important; }</style>', unsafe_allow_html=True)

# تنسيق القسم العلوي (Hero)
...
st.markdown('<style>.hero-title { font-size: 46px !important; font-weight: 800 !important; color: #5B21B6 !important; margin: 0 !important; line-height: 1.2; } .hero-subtitle { font-size: 18px !important; font-weight: 700 !important; color: #1E293B !important; margin-top: 14px !important; margin-bottom: 14px !important; text-align: center !important; }</style>', unsafe_allow_html=True)
st.markdown('<style>.hero-description { font-size: 15px !important; color: #64748B !important; max-width: 650px; margin: 0 auto 24px auto !important; line-height: 1.7; text-align: center !important; } .author-badge { display: inline-block; background: #F3E8FF; color: #6B21A8 !important; padding: 6px 20px; border-radius: 100px; font-size: 13px !important; font-weight: 600; border: 1px solid #E9D5FF; }</style>', unsafe_allow_html=True)

# تنسيق البطاقات وصندوق الإدخال
st.markdown('<style>.section-card { background: #FFFFFF; border: 1px solid #F1F5F9; border-radius: 24px; padding: 35px; box-shadow: 0 4px 25px rgba(0, 0, 0, 0.015); margin-top: 30px; margin-bottom: 5px; text-align: right !important; }</style>', unsafe_allow_html=True)
...
st.markdown('<style>.stTextArea textarea { background-color: #FFFFFF !important; border: 1px solid #E2E8F0 !important; border-radius: 16px !important; padding: 18px !important; font-size: 14.5px !important; color: #334155 !important; text-align: right !important; direction: rtl !important; }</style>', unsafe_allow_html=True)
st.markdown('<style>div.stButton > button { background: #6D28D9 !important; color: white !important; font-weight: 600 !important; font-size: 14.5px !important; border-radius: 12px !important; border: none !important; padding: 10px 24px !important; box-shadow: 0 4px 12px rgba(109, 40, 217, 0.25) !important; }</style>', unsafe_allow_html=True)

# تنسيق بطاقات الحالات الفارغة (لم يتم إجراء أي تحليل بعد)
st.markdown('<style>.inner-dashed-box { border: 1px dashed #E2E8F0; border-radius: 16px; padding:
