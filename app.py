...
# بطاقة عرض النتائج الشاملة
st.markdown('<div class="section-card">', unsafe_allow_html=True)
st.markdown('<h3 class="card-title-text">📊 نتيجة التحليل الدلالي</h3>', unsafe_allow_html=True)
            ...
            with st.spinner("⏳ يقوم لبيب بتحليل السياق الدلالي عبر نموذج AraBERT..."):
                ...
                if user_vector is not None:
                    ...
                    if similarities:
                        ...
                        st.success(f"🎯 المعنى المكتشف للكلمة ({detected_word}) في النص هو: {best_meaning}")
                        st.progress(int(confidence))
                        st.markdown(f"**نسبة المطابقة السياقية والثقة:** {confidence}%")

                        # تحويل وتجهيز جدول البيانات للعرض
                        df = pd.DataFrame(similarities)
                        df["التشابه"] = df["التشابه"].apply(lambda x: f"{round(x*100, 2)}%")
                        st.table(df)
                    else:
                        st.error("تعذر حساب معاملات التشابه للمتجهات.")
                else:
                    st.error("تعذر استخراج المتجهات الدلالية للكلمة المستهدفة من النص الحالي.")
        else:
            st.warning("⚠️ لم يتم العثور على كلمة مدعومة حالياً داخل النص (الكلمات المدعومة: عين، المغرب، رأس).")
    else:
        st.warning("⚠️ يرجى إدخال نص عربي أولاً.")
else:
    st.markdown("""
    <div class="empty-box">
        <h3 style="color:#6D28D9; font-size:16px; font-weight:700;">🔍 لم يتم إجراء أي تحليل بعد</h3>
        <p style="color:#64748B; font-size:14px; margin:5px 0 0 0;">أدخل نصاً عربياً يحتوي على كلمة مدعومة واضغط على زر التحليل للحصول على النتيجة السياقية.</p>
    </div>
...
with col1:
    st.markdown("""
    <div class="step-card">
        <h3 style="color:#6D28D9; font-size:16px; font-weight:700; margin-bottom:8px;">🔍 تحليل السياق</h3>
        <p style="color:#64748B; font-size:13.5px; line-height:1.6;">يقوم لبيب بتحليل الكلمات المحيطة باللفظ المشترك عبر شبكة انتباه نموذج AraBERT لفهم السياق اللغوي.</p>
    </div>
    """, unsafe_allow_html=True)
...
with col2:
    st.markdown("""
    <div class="step-card">
        <h3 style="color:#7C3AED; font-size:16px; font-weight:700; margin-bottom:8px;">🎯 اكتشاف المعنى</h3>
        <p style="color:#64748B; font-size:13.5px; line-height:1.6;">يحدد المعنى الدقيق والأقرب للنص المدخل اعتماداً على قيم التضمين السياقي والدلالة المعجمية.</p>
    </div>
    """, unsafe_allow_html=True)
...
with col3:
    st.markdown("""
    <div class="step-card">
        <h3 style="color:#2563EB; font-size:16px; font-weight:700; margin-bottom:8px;">📊 قياس التشابه</h3>
        <p style="color:#64748B; font-size:13.5px; line-height:1.6;">يقارن مصفوفة المتجهات الحالية مع عينات القاموس الدلالي المرجعي عبر خوارزمية جيب التمام (Cosine Similarity).</p>
    </div>
    """, unsafe_allow_html=True)
